...
export default AjaxService.extend({
  ...
  request(url, options) {
    if (options && 'live' in options) {
      console.warn("DEPRECATION: live option is deprecated, use lifetime instead");
      options.lifetime = options.live;
    }

    if (options && 'lifetime' in options) {
      let inCacheValue = this.cache.get(url);
      if (inCacheValue != null) {
        return new Promise(resolve => resolve(inCacheValue));
      }
    }
